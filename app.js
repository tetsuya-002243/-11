...
let __lastTouchEnd=0;document.addEventListener("touchend",e=>{let now=Date.now();if(now-__lastTouchEnd<=300)e.preventDefault();__lastTouchEnd=now;},{passive:false});
if("serviceWorker"in navigator){addEventListener("load",()=>navigator.serviceWorker.register("./service-worker.js").catch(()=>{}))}
pdfjsLib.GlobalWorkerOptions.workerSrc="https://cdnjs.cloudflare.com/ajax/libs/pdf.js/3.11.174/pdf.worker.min.js";
const $=id=>document.getElementById(id);
let db; const DB="fieldbook-v10-db";
const st={currentBridge:null,currentPdfId:null,tool:"move",sel:null,pending:null,relocate:null,pdfDoc:null,page:1,pages:0,w:0,h:0,scale:1,tx:0,ty:0,pts:new Map(),pan:null,pinch:null,drawing:false,stroke:null,down:null,photoDown:null,photoPlacementLocked:false,continuousIconId:null,history:["homePage"]};
...
$("homeEditBtn").onclick=()=>{show("editorPage");renderPdfSelect()};
$("homeSyncBtn").onclick=()=>show("syncPage");
$("bridgeSearch").oninput=()=>renderBridgeList();
...
function renderBridgeList(){
  let a=$("bridgeList");
  let q=($("bridgeSearch")?.value||"").trim().toLowerCase();
  let ids=Object.keys(app.bridges).filter(id=>app.bridges[id].name.toLowerCase().includes(q));
  a.innerHTML=ids.length?"":"<p class='note'>まだ橋梁がありません。</p>";
  ids.forEach(id=>{
    let b=app.bridges[id],pending=b.photos.filter(p=>p.sync!=="done").length;
    let div=document.createElement("div");
    div.className="bridgeItem"+(id===st.currentBridge?" active":"");
    div.innerHTML=`<div class="bridgeTitle">${b.name}</div>
      <div class="bridgeStats">PDF ${b.pdfs.length}件 / 写真 ${b.photos.length}枚 / 未同期 ${pending}枚</div>
      <div class="bridgeActions"></div>`;
    let actions=div.querySelector(".bridgeActions");
    let open=document.createElement("button");open.textContent="開く";open.onclick=()=>{st.currentBridge=id;save();renderBridgeList();updateCurrent();show("pdfPage")};
    let edit=document.createElement("button");edit.textContent="編集";edit.onclick=async()=>{st.currentBridge=id;save();renderBridgeList();updateCurrent();show("editorPage");renderPdfSelect();if(b.pdfs[0])await openPdf(b.pdfs[0].id)};
    let sync=document.createElement("button");sync.textContent="写真保存";sync.onclick=()=>{st.currentBridge=id;save();renderBridgeList();updateCurrent();show("syncPage")};
    actions.append(open,edit,sync);
    a.appendChild(div);
  })
}
...
async function addPdfFiles(files){
  let b=currentBridge();
  if(!b)return alert("先に橋梁を作成または選択してください");
  for(let f of [...files]){
    let id=uid(),key=`pdf_${b.id}_${id}`;
    await putFile(key,f);
    b.pdfs.push({id,name:f.name,key,checked:{},icons:[],draw:[],texts:[]});
  }
  save();
  renderBridgeList();
  renderPdfList();
...
$("pdfInput").onchange=async e=>{await addPdfFiles(e.target.files);e.target.value=""}
$("homePdfInput").onchange=async e=>{await addPdfFiles(e.target.files);e.target.value=""}
...
function setTool(t){
  ...
  st.relocate=null;
  st.photoDown=null;
  st.photoPlacementLocked=false;
  document.querySelectorAll(".tool").forEach(b=>b.classList.remove("active"));
  $(t).classList.add("active");
  $("inkCanvas").style.pointerEvents=(["pen","marker","eraser"].includes(t))?"auto":"none";
}
["move","pen","marker","text","eraser"].forEach(id=>$(id).onclick=()=>setTool(id));
$("photoIcon").onclick=()=>{
  setTool("photoIcon");
  st.photoPlacementLocked=false;
  st.photoDown=null;
...
$("view").onpointerdown=e=>{
  ...
  st.down={x:e.clientX,y:e.clientY,t:Date.now(),tool:st.tool,pointerId:e.pointerId};
  let p=toPdf(e.clientX,e.clientY);

  if(st.relocate){moveIconTo(st.relocate,p);return}

  if(st.tool==="photoIcon"){
    if(st.photoPlacementLocked)return;
    if(!e.isPrimary){return}
    st.photoDown={x:e.clientX,y:e.clientY,p,created:false,pointerId:e.pointerId};
    return;
  }

  if(st.tool==="text"){addText(p);return}
  if(["pen","marker","eraser"].includes(st.tool)){startStroke(e);return}
...
$("view").onpointermove=e=>{
  ...
  if(st.tool==="photoIcon")return;

  if(["pen","marker","eraser"].includes(st.tool)&&st.drawing){moveStroke(e);return}
  if(st.tool!=="move")return;

  let a=[...st.pts.values()];
  if(a.length===1&&st.pan){
    st.tx+=e.clientX-st.pan.x;st.ty+=e.clientY-st.pan.y;
    st.pan={x:e.clientX,y:e.clientY};tf()
  }else if(a.length>=2){
    let d=Math.hypot(a[0].x-a[1].x,a[0].y-a[1].y),c={x:(a[0].x+a[1].x)/2,y:(a[0].y+a[1].y)/2};
    if(st.pinch){
      let before=toPdf(c.x,c.y);
      st.scale=Math.min(6,Math.max(.15,st.scale*d/st.pinch.d));
      let r=$("view").getBoundingClientRect();
      st.tx=c.x-r.left-before.x*st.scale;
      st.ty=c.y-r.top-before.y*st.scale;
      tf()
    }
    st.pinch={d,c}
  }
}
$("view").onpointerup=$("view").onpointercancel=e=>{
  if(["pen","marker","eraser"].includes(st.tool)&&st.drawing)endStroke();

  if(st.tool==="photoIcon"&&st.photoDown&&st.photoDown.pointerId===e.pointerId&&!st.photoPlacementLocked){
    let dx=e.clientX-st.photoDown.x,dy=e.clientY-st.photoDown.y;
    let isTap=Math.hypot(dx,dy)<14;
    if(isTap){
      st.photoPlacementLocked=true;
      let p=st.photoDown.p;
      st.photoDown=null;
      createIcon(p);
    }else{
      st.photoDown=null;
  ...
  }else if(st.down&&st.down.tool==="move"&&st.pts.size===1&&!st.relocate){
    let dx=e.clientX-st.down.x,dy=e.clientY-st.down.y,dt=Date.now()-st.down.t;
    if(Math.abs(dx)>120&&Math.abs(dx)>Math.abs(dy)*1.4&&dt<900)goPage(dx<0?st.page+1:st.page-1)
  }
...
function actionStack(){
  let p=currentPdf();
  if(!p)return null;
  if(!p.actionStack)p.actionStack=[];
  return p.actionStack;
}
function recordAction(a){
  let s=actionStack();
  if(!s)return;
  s.push(a);
  if(s.length>150)s.shift();
}
function createIcon(p){
  ...
  if(!pdf)return;
  let id=uid();
  pdf.icons.push({id,no:pdf.icons.length+1,page:st.page,x:p.x/st.w,y:p.y/st.h,photoIds:[]});
  recordAction({type:"addIcon",id});
  st.sel=id;
  renderAll();
  save();
  setTool("move");
  openCameraForIcon(id);
}

function openCameraForIcon(iconId){
  st.pending=iconId;
  st.continuousIconId=iconId;
  setTimeout(()=>$("autoPhoto").click(),120);
}
$("autoPhoto").onchange=async e=>{
  let ic=findIcon(st.pending);
  let files=[...e.target.files];
  if(ic&&files.length){
    await addFiles(ic,files);
    openPhotos(ic.id);
  }
  st.pending=null;
  st.photoPlacementLocked=false;
...
async function addFiles(icon,files){let b=currentBridge(),newIds=[];for(let f of files){let src=await resize(f),seq=b.photoSeq++,name=`${safe(b.name)}_${String(seq).padStart(3,"0")}.jpg`,id=uid();b.photos.push({id,name,src,sync:"pending"});icon.photoIds.push(id);newIds.push(id)}if(newIds.length)recordAction({type:"addPhotos",iconId:icon.id,photoIds:newIds});save();renderAll();openPhotos(icon.id);renderSync()}
...
function moveIconTo(id,p){let i=findIcon(id);if(!i)return;recordAction({type:"moveIcon",id,old:{page:i.page,x:i.x,y:i.y}});i.page=st.page;i.x=p.x/st.w;i.y=p.y/st.h;st.relocate=null;setTool("move");st.sel=id;renderAll();save();alert("位置を変更しました")}
function addText(p){let t=prompt("記入文字");if(!t)return;currentPdf().texts.push({page:st.page,x:p.x/st.w,y:p.y/st.h,text:t,color:$("penColor").value});recordAction({type:"text"});renderAll();save()}
...
function endStroke(){if(st.stroke&&st.stroke.pts.length>1){currentPdf().draw.push(st.stroke);recordAction({type:"draw"})}st.stroke=null;st.drawing=false;drawInk();save()}
$("undo").onclick=()=>{
  let p=currentPdf(),b=currentBridge();if(!p)return;
  let s=actionStack(),a=s&&s.pop();
  if(!a){
    if(p.draw.length)p.draw.pop();else if(p.texts.length)p.texts.pop();else return alert("戻せる作業がありません");
  }else if(a.type==="draw"){p.draw.pop()
  }else if(a.type==="text"){p.texts.pop()
  }else if(a.type==="addIcon"){
    let ic=p.icons.find(i=>i.id===a.id);
    if(ic&&b){b.photos=b.photos.filter(ph=>!ic.photoIds.includes(ph.id))}
    p.icons=p.icons.filter(i=>i.id!==a.id);p.icons.forEach((i,n)=>i.no=n+1);st.sel=null
  }else if(a.type==="addPhotos"){
    let ic=p.icons.find(i=>i.id===a.iconId);
    if(ic)ic.photoIds=ic.photoIds.filter(id=>!a.photoIds.includes(id));
    if(b)b.photos=b.photos.filter(ph=>!a.photoIds.includes(ph.id))
  }else if(a.type==="moveIcon"){
    let ic=p.icons.find(i=>i.id===a.id);if(ic){ic.page=a.old.page;ic.x=a.old.x;ic.y=a.old.y}
  }
  renderAll();renderSync();save()
}
...
function icons(){let p=currentPdf(),l=$("icons");l.innerHTML="";if(!p)return;p.icons.filter(i=>i.page===st.page).forEach(i=>{let ps=iconPhotos(i),d=document.createElement("div");d.className="ico"+(i.id===st.sel?" selected":"")+(ps.length?" hasPhoto":" noPhoto");d.style.left=i.x*st.w+"px";d.style.top=i.y*st.h+"px";d.innerHTML=ps.length?`<img src="${ps[0].src}"><span class="num">${i.no}</span>`:`<span class="num">${i.no}</span>`;d.onclick=e=>{e.stopPropagation();st.sel=i.id;renderAll();openPhotos(i.id)};l.appendChild(d)});texts()}
...
function setPhotoButtonLabel(count){
  let btn=$("modalAddPhoto");
  if(!btn)return;
  btn.textContent=count===0?"撮影":"続けて撮影";
}
function openPhotos(id){
  let i=st.icons?null:null; // no-op guard for older cache
  let ic=findIcon(id);
  if(!ic)return;
  let ps=iconPhotos(ic);
  $("modalTitle").textContent=`写真アイコン${ic.no}：${ps.length}枚`;
  setPhotoButtonLabel(ps.length);
  $("modalPhotos").innerHTML=ps.length?ps.map(p=>`<div class="photoBox"><img src="${p.src}"><span>${p.name}</span><span class="sync ${p.sync==='done'?'done':''}">${p.sync==='done'?'同期済み':'未同期'}</span></div>`).join(""):"<p>まだ写真がありません。撮影ボタンで写真を追加してください。</p>";
  $("photoModal").classList.remove("hidden");
}
$("closePhoto").onclick=()=>$("photoModal").classList.add("hidden");$("modalAddPhoto").onclick=()=>{let ic=findIcon(st.sel);if(!ic)return alert("写真アイコンを選択してください");$("photoModal").classList.add("hidden");openCameraForIcon(ic.id)};$("modalMoveIcon").onclick=()=>{if(st.sel){st.relocate=st.sel;$("photoModal").classList.add("hidden");alert("新しい位置をPDF上で1回タップしてください")}};$("modalDeleteIcon").onclick=()=>{let p=currentPdf();if(!p||!st.sel)return;if(confirm("削除しますか？")){p.icons=p.icons.filter(i=>i.id!==st.sel);p.icons.forEach((i,n)=>i.no=n+1);st.sel=null;$("photoModal").classList.add("hidden");renderAll();save()}};
...
function emptyPhotoIcons(){
  let b=currentBridge(),items=[];
  if(!b)return items;
  b.pdfs.forEach(pdf=>{
    (pdf.icons||[]).forEach(ic=>{
      if(!ic.photoIds||ic.photoIds.length===0)items.push({pdf:pdf.name,no:ic.no,page:ic.page});
    });
  });
  return items;
}
function validateNoEmptyPhotoIcons(){
  let empty=emptyPhotoIcons();
  if(empty.length){
    alert("写真が入っていない写真アイコンがあります。写真を追加するか、不要なアイコンを削除してください。\n\n"+empty.map(x=>`${x.pdf} ${x.page}ページ アイコン${x.no}`).join("\n"));
    return false;
  }
  return true;
}
function renderSync(){let b=currentBridge();if(!b){$("syncSummary").textContent="橋梁未選択";$("photoSummary").innerHTML="";return}let pending=b.photos.filter(p=>p.sync!=="done").length,done=b.photos.filter(p=>p.sync==="done").length,empty=emptyPhotoIcons().length;$("syncSummary").innerHTML=`橋梁：<b>${b.name}</b><br>写真：${b.photos.length}枚　未同期：${pending}枚　同期済み：${done}枚　写真なしアイコン：${empty}個`;$("photoSummary").innerHTML=b.photos.map(p=>`<div class="photoBox"><img src="${p.src}"><span>${p.name}</span><span class="sync ${p.sync==='done'?'done':''}">${p.sync==='done'?'同期済み':'未同期'}</span></div>`).join("")}
function folderName(){let b=currentBridge();return safe($("folderName").value||b?.name||"橋梁写真")}
async function exportPhotoZip(){
  if(!validateNoEmptyPhotoIcons())return;
  let b=currentBridge();
